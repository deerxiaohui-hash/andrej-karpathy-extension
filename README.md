...
- **过度工程时引导。** 一次 `write`/`edit` 后，统计新引入的顶层抽象（函数、类、接口、类型；嵌套声明不计入）。对 `write` 会先减去文件里原本就有的声明，只算净新增。如果太多，在 tool_result 中追加警告让模型重新考虑。
...
一次改动引入过多顶层抽象时，模型会在下一轮收到（这条在界面上不可见，只给模型看）：

```text
⚠️ [Karpathy] 你刚刚引入了 6 个新的顶层抽象（阈值：2）。
检测到：function `foo`, function `bar`, class `Baz`, ...
```
